...
class Ledger:
    ...
    def issuer(self, unique_id):
        return self.get(unique_id).issuer
    
    def holder(self, unique_id):
        return self.get(unique_id).holder
    
    def records_by_issuer(self, issuer):
        return self.df.iloc[np.where(self.df.issuer == issuer)[0]]
    
    def records_by_holder(self, holder):
        return self.df.iloc[np.where(self.df.holder == holder)[0]]
...
class DepositLedger(Ledger):
    ...
    def current_balance(self, unique_id):
        return self.get(unique_id).value
    ...
    def credit(self, unique_id, value):        
        if self.account_exists(unique_id):
            self.df.loc[unique_id, 'value'] += value
            return True
        else:
            raise ValueError(f'ID {unique_id} does not exist in this ledger')
    ...
    def apply_daily_interest(self, issuer=None):
        ...
        daily_rate = (self.df.loc[indexes, 'interest_rate']/100.0)/self.model.days_in_year
...
class LoanLedger(Ledger):
    ...
    def extend_loan(self, unique_id, value, maturity_days):        
        if self.account_exists(unique_id):
            self.df.loc[unique_id, 'value'] += value
            self.df.loc[unique_id, 'maturity_date'] = self.model.day + maturity_days
            return True
        else:
            raise ValueError(f'ID {unique_id} does not exist in this ledger')
    ...
    def revalue_all(self):
        ...
        self.df.hold_to_maturity_value = self.df.value * ((1 + (self.df.interest_rate/100))/self.model.days_in_year)**(self.maturity_date-self.model.day)
...
class BondLedger(Ledger):
    ...
    def coupon_interval_days(self):
        return self.model.days_in_year/self.annual_coupon_frequency
    
    @property 
    def is_short_term(self):
        return self.df.maturity_days < self.model.days_in_year
    ...
    def create(self, issuer, interest_rate, maturity_years):
        ...
        if maturity_years < 0.75:
            interest_rate = 0
            issue_date = self.model.start_of_this_month
            ...
            maturity_days    = np.ceil(maturity_years*self.model.days_in_year)
            maturity_date    = issue_date + maturity_days
            days_to_maturity = maturity_date - self.model.day
            number_of_outstanding_coupon_payments = 0
            next_coupon_date = np.nan
            mark_to_market_value   = value # Change this to reflect market price and coupon payments.
            hold_to_maturity_value = value 
            
        else:
            issue_date = self.model.start_of_this_year
    ...
    def transfer(self, idval, new_holder):
        ...
        if bond.holder != new_holder:
            self.df.loc[idval,'holder'] = new_holder            
        else:
            raise ValueError('Bond already owned by new holder')
    
    def close(self, idval):
        bond = self.get(idval)
        current_holder = bond.iloc[0]['holder']
        
        if current_holder == self.issuer:
            return self.drop(idval)
        else:
            return False
        
    def recalculate(self):    
        self.df['days_to_maturity'] = self.df.maturity_date - self.model.day
        ...
        self.df['mark_to_market_value']   = self.df.value # Change this to reflect market price and coupon payments.
        self.df['hold_to_maturity_value'] = self.df.value + (self.df.value * (((self.df.interest_rate/100))/self.annual_coupon_frequency) * self.df.number_of_outstanding_coupon_payments)
        return True
