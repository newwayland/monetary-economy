# -*- coding: utf-8 -*-
"""
Created on Wed Jul  5 15:33:58 2023
...
def planned_consumption_amount(income, current_liquidity: int, average_price: float) -> float:
    """
    ...
    modigliani = False
    if modigliani:
        savings = np.max([0, current_liquidity-income])
        spending = HouseholdConfig.alpha_1 * income + HouseholdConfig.alpha_2*savings
        return (spending / average_price)
    else:
        return np.min([(current_liquidity / average_price) ** HouseholdConfig.alpha, (current_liquidity / average_price)])


# AGENT
class Household(Individual):  
    ...
    __slots__ = ('model', 'unique_id', 'reservation_wage', 'preferred_suppliers', 'employer',
                 'blackmarked_firms', 'looked_for_cheaper_vendor', 'found_cheaper_vendor',
                 'looked_for_better_vendor', 'found_better_vendor', 'vendor_already_replaced',
                 'looked_for_new_job', 'found_new_job', 'average_goods_price', 'planned_consumption',
                 'current_demand', 'planned_saving', 'poverty', 'unsatisfied_demand',
                 'demand_constraints_suffered', 'income')
    ...
    def __init__(self, model) -> None:
        ...
        self.preferred_suppliers = random.choices(
            model.firms,
            k=HouseholdConfig.num_preferred_suppliers
        )
        self.employer = None
    ...
    def month_start(self) -> None:
        ...
        self.income = self.employer.wage_rate if not self.is_unemployed else 0
...
# MONTH START
    ...
    def find_cheaper_vendor(self) -> None:
        ...
        target_index = random.choice(supplier_index)
        change_price = (
    ...
    def plan_consumption(self) -> None:
        ...
        """
        self.average_goods_price = np.mean([o.goods_price for o in self.preferred_suppliers])
        ...
        try:
            ...
            self.current_demand = (
                self.planned_consumption // self.model.schedule.days_in_month
            )
            self.planned_saving = (
                self.deposit_balance -
            ...
            self.planned_saving = self.deposit_balance

# DAILY

    def rank_preferred_suppliers(self):
        # Put the preferred suppliers in a random order
        self.preferred_suppliers.sort(key=attrgetter("goods_price"))
        
    def shuffle_preferred_suppliers(self):
        random.shuffle(self.preferred_suppliers)
    ...
    def buy_goods(self) -> None:
        ...
        self.shuffle_preferred_suppliers()
        
        # Obtain the required amount of goods from
        # the preferred suppliers
        required_amount = self.current_demand
        satisfaction_amount = math.floor(required_amount * (1 - HouseholdConfig.satisfaction_fraction))
...
# HELPERS
    ...
    def get_affordable_amount(self, firm) -> int:
        ...
        """
        try:
            return self.deposit_balance // firm.goods_price
        except ZeroDivisionError:
            return math.inf
    ...
    def transact(self, firm, quantity: int, total_price: int) -> None:
        ...
        """
        self.pay(firm.deposit_account_number, total_price)
    ...
    def select_new_firm(self):
        ...
        """
        result = random.choice(self.model.firms)
    ...
    def select_blackmarked_firm(self):
        ...
        return random.choices(firm_list, weights=blackmarks)[0]
